...
import {AsyncDataOptions, FetchResult, UseFetchOptions, useNuxtApp} from '#app'
...
// This break the "pick" strategy from nuxt
...
// We add the ReturnType in the generic so we can type hint API return + we use our own wrapper appFetch
export default async function<
    ...
    const asyncData = useAsyncData<_ResT, ErrorT, Transform, PickKeys>(key, () => {
        ...
        return $appFetch(_request.value, { signal: controller.signal, ..._fetchOptions } as any) as Promise<_ResT>
    }, _asyncDataOptions)
